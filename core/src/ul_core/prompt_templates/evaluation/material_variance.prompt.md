...
For response findings, ignore wording, formatting, and added non-conflicting explanation, signatures, or apologies. Treat a changed factual assertion, recommendation, refusal, requested next step, recipient, or required routing marker as material. For example, removing `[PRIORITY]` from a subject is material even when the broad send action is unchanged.
