...
"$UL_QUALIFICATION_ROOT/tmp/ul-55/venv/bin/ul" probe \
  "$UL_QUALIFICATION_ROOT/cli/tests/fixtures/probe_qualification_interactions.jsonl" \
  --target probe_qualification_targets:invoke \
  --target-working-directory "$UL_QUALIFICATION_ROOT/cli/tests/fixtures" \
...
export UL_QUALIFICATION_RECEIPT="$PWD/target-calls.jsonl"
PYTHONPATH="$UL_QUALIFICATION_ROOT/cli/tests/fixtures" \
  "$UL_QUALIFICATION_ROOT/tmp/ul-55/venv/bin/python" \
  -m probe_qualification_targets --port 8765
```
...
state verification as unavailable rather than passed. See [`ul-55.md`](ul-55.md) for the measured
result and the credential-dependent finding gate.
