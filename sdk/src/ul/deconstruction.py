    ...
    async def deconstruct(
        ...
        response = await self._request(
            ...
            system_prompt=(
                ...
                "structured output object. An action is invalid if that support is missing; "
                "input evidence or one container pointer cannot replace it. Every action outcome "
                "must list the request unit IDs that it fulfills; if the action cannot be linked, "
                "mark it unresolved. "
                "Container evidence does not support an action. "
